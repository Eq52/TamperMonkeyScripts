...
(function() {
    ...
    const Interface = {
        ...
        buildMenu: function() {
            ...
            if (pageType === 'video') {
                h += `<div class="xj-item" id="xj-parse-nx">🚀 极速解析 (NX)<span class="xj-tag">Fast</span></div>`;
    ...
    const Driver = {
        ...
        startNxvav: async function() {
            try {
                ...
                Interface.msg('⏳ 正在解析...');
                const apiUrl = `https://api.nxvav.cn/api/bilivideo/?bv=${vInfo.id}&q=80&format=mp4&otype=json`;

                const resp = await fetch(apiUrl);
                const json = await resp.json();

                if (json.code === 0 && json.url) {
                    const title = document.querySelector('h1.video-title, .video-info-title')?.textContent?.trim() || vInfo.id;
                    const simUrl = `https://eq52.github.io/Sim-Player/?url=${encodeURIComponent(json.url)}&title=${encodeURIComponent(title)}&rf=dnc`;
                    const box = this.getContainer();
                    this.injectIframe(box, simUrl);
                    Interface.msg('✅ 解析成功');
                } else {
