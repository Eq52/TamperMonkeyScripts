...
// @version      6.0.0
// @description  B站PC手机双端适配解析，番剧解析(第三方API)，视频下载(第三方API)，强力去广告，使用第三方API在未登录情况下体验高清画质。集成SimPlayer播放器，解析后使用独立播放器播放视频。
            ...
            d.innerHTML = `
                <div id="xj-set-box">
                    ...
                    <p style="font-size:12px;color:#888;margin-bottom:5px;">视频页锁定专用接口，此处仅配置番剧页备用接口</p>
                                        <p style="font-size:12px;color:#888;margin-bottom:5px;">如果都解析不了可尝试:http://4275.com/5zk25s</p>
    ...
    const Driver = {
        ...
        startNxvav: async function() {
            try {
                ...
                Interface.msg('⏳ 正在解析视频地址...');

                // 使用 JSON 输出获取真实视频地址，再交给 SimPlayer 播放
                const apiUrl = `https://api.nxvav.cn/api/bilivideo/?bv=${vInfo.id}&q=80&otype=json`;
                const resp = await fetch(apiUrl);
                const json = await resp.json();

                if (json.code === 0 && json.url) {
                    // 提取视频标题
                    const title = document.querySelector('.video-title, #viewbox_report h1, .bilibili-player-video-top-title, .video-info-title')?.textContent?.trim() || ('BV' + vInfo.id);
                    const simPlayerUrl = 'https://eq52.github.io/Sim-Player/?url=' + encodeURIComponent(json.url) + '&title=' + encodeURIComponent(title);
                    const box = this.getContainer();
                    this.injectIframe(box, simPlayerUrl);
                    Interface.msg('✅ SimPlayer 播放中');
                } else {
